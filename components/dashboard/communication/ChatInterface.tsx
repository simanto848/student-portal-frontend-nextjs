...
import { Send, Paperclip, CheckCheck, Loader2, Search, MoreVertical, Pencil, Trash2, Pin, Info, X, Image as ImageIcon, FileText } from "lucide-react";
...
export function ChatInterface({ chatGroupId, chatGroupType = 'group', initialMessages = [] }: ChatInterfaceProps) {
    ...
    useEffect(() => {
        if (chatGroupId) {
            setLoading(true);
            fetchMessages();
            const interval = setInterval(() => fetchMessages(false), 5000); // Polling every 5s, silent update
    ...
    const fetchMessages = async (showLoading = true) => {
        ...
        try {
            const data = await chatService.getMessages(chatGroupId, 50, 0, searchQuery);
    ...
    const handleSendMessage = async (e?: React.FormEvent) => {
        ...
        try {
            ...
            setNewMessage("");
            fetchMessages(false); // Refresh to respect server timestamps/order
        } catch (error) {
            ...
            <div className="px-4 py-2 bg-white border-b flex items-center gap-2 justify-between">
                ...
                <Sheet open={isInfoOpen} onOpenChange={setIsInfoOpen}>
                    ...
                    <SheetContent>
                        ...
                        <Tabs defaultValue="pinned" value={infoTab} onValueChange={setInfoTab} className="mt-4 h-full flex flex-col">
                            <TabsList className="grid w-full grid-cols-2">
                                <TabsTrigger value="pinned">Pinned</TabsTrigger>
                                <TabsTrigger value="media">Media</TabsTrigger>
                            </TabsList>
                            <TabsContent value="pinned" className="flex-1 overflow-auto mt-4 px-1">
                                {loadingInfo ? (
                                    <div className="flex justify-center py-8"><Loader2 className="animate-spin text-muted-foreground" /></div>
                                ) : pinnedMessages.length === 0 ? (
                                    <div className="text-center text-muted-foreground py-8 text-sm">No pinned messages</div>
                                ) : (
                                    <div className="space-y-3">
                                        {pinnedMessages.map(msg => (
                                            <div key={msg.id} className="bg-slate-50 p-3 rounded-lg border text-sm">
                                                ...
                                                <p className="text-slate-700 line-clamp-3">{msg.content}</p>
                                            </div>
                                        ))}
                                    </div>
                                )}
                            </TabsContent>
                            <TabsContent value="media" className="flex-1 overflow-auto mt-4 px-1">
                                {loadingInfo ? (
                                    <div className="flex justify-center py-8"><Loader2 className="animate-spin text-muted-foreground" /></div>
                                ) : mediaMessages.length === 0 ? (
                                    <div className="text-center text-muted-foreground py-8 text-sm">No media files</div>
                                ) : (
                                    <div className="grid grid-cols-2 gap-2">
                                        {/* Placeholder for media rendering - as backend service implies attachments exist */}
                                        {mediaMessages.map(msg => (
                                            msg.attachments?.map((att, i) => (
                                                <div key={`${msg.id}-${i}`} className="aspect-square bg-slate-100 rounded-md flex items-center justify-center border">
            ...
            <div className="flex-1 overflow-hidden relative">
                        ...
                        {messages.length === 0 ? (
                            ...
                            messages.map((msg, idx) => {
                                ...
                                return (
                                    <div
                                        key={msg.id || idx}
                                        className={`flex gap-3 group ${isMe ? "justify-end" : "justify-start"} ${isContinuous ? "mt-1" : "mt-4"}`}
                                    >
                                        {!isMe && !isContinuous && (
                                        ...
                                        <div className={`flex flex-col ${isMe ? "items-end" : "items-start"} max-w-[70%]`}>
                                            ...
                                            {isDeleting ? (
                                                <div className="bg-red-50 border border-red-200 rounded-xl p-3 flex items-center gap-3 animate-in fade-in zoom-in-95 duration-200">
                                                    <p className="text-xs text-red-800 font-medium">Delete this message?</p>
                                                    <div className="flex gap-2">
                                                        <Button size="xs" variant="ghost" className="text-red-600 hover:text-red-700 hover:bg-red-100" onClick={() => setDeletingMessageId(null)}>Cancel</Button>
                                                        <Button size="xs" variant="destructive" onClick={() => handleDeleteMessage(msg.id)}>Delete</Button>
                                                    </div>
                                                </div>
                                            ) : (
                                                <div
                                                    className={`relative px-4 py-2 shadow-sm ${isMe
                                                ...
                                                <DropdownMenuContent align={isMe ? "end" : "start"}>
                                                    ...
                                                    {isMe && (
                                                        <>
                                                            ...
                                                            <DropdownMenuItem className="text-red-600 focus:text-red-600" onClick={() => setDeletingMessageId(msg.id)}>
                ...
                {editingMessageId && (
                    <div className="absolute bottom-0 left-0 right-0 bg-white/90 backdrop-blur-sm border-t px-4 py-2 flex justify-between items-center text-xs text-amber-700 z-10">
                        <span className="font-semibold flex items-center gap-2"><Pencil className="h-3 w-3" /> Editing Message</span>
                        <Button variant="ghost" size="sm" onClick={cancelEditing} className="h-6 w-6 p-0 hover:bg-slate-200 rounded-full"><X className="h-3 w-3" /></Button>
                    </div>
                )}
            </div>

            {/* Input Area */}
            <div className={`p-4 bg-white border-t transition-colors ${editingMessageId ? 'bg-amber-50/50' : ''}`}>
                <form onSubmit={handleSendMessage} className="flex items-center gap-2">
                    ...
                    <Input
                        ...
                        className={`flex-1 rounded-full border-slate-200 py-5 ${editingMessageId ? 'focus-visible:ring-amber-500 border-amber-200' : 'focus-visible:ring-[#1a3d32]'}`}
                        disabled={sending}
                    />
                    <Button
                        type="submit"
                        size="icon"
                        className={`rounded-full w-12 h-10 ${editingMessageId ? 'bg-amber-600 hover:bg-amber-700' : 'bg-[#1a3d32] hover:bg-[#142e26]'}`}
                        disabled={!newMessage.trim() || sending}
                    >
                        {sending ? <Loader2 className="h-5 w-5 animate-spin" /> : (
                            editingMessageId ? <CheckCheck className="h-5 w-5 ml-0.5" /> : <Send className="h-5 w-5 ml-0.5" />
                        )}
                    </Button>
                </form>
            </div>
