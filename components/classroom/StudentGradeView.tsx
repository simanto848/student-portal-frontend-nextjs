...
export function StudentGradeView({
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const data = await courseGradeService.list({
        courseId,
        batchId,
        studentId,
      });
      // Take the first one as it's per course/student
          ...
          {isLoading ? (
            ...
            <div className="grid gap-6 md:grid-cols-3">
              <div className="flex flex-col items-center justify-center p-6 bg-gray-50 rounded-lg">
                ...
                <span className="text-3xl font-bold text-[#1a3d32]">
                  {grade.totalMarks}
                </span>
              </div>
              <div className="flex flex-col items-center justify-center p-6 bg-gray-50 rounded-lg">
              ...
              <div className="flex flex-col items-center justify-center p-6 bg-[#3e6253]/10 rounded-lg border border-[#3e6253]/20">
                <span className="text-sm text-[#3e6253] font-medium mb-1">
                  Final Grade
                </span>
                <span className="text-4xl font-bold text-[#3e6253]">
                  {grade.grade}
                </span>
              </div>
