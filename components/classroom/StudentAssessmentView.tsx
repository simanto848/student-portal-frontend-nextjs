...
export function StudentAssessmentView({ courseId, batchId, studentId }: StudentAssessmentViewProps) {
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {isLoading ? (
                    ...
                    assessments.map((assessment) => {
                        ...
                        const isLate = assessment.dueDate && new Date() > new Date(assessment.dueDate) && !isSubmitted;
                        ...
                        return (
                            <Card key={assessment.id} className="flex flex-col">
                                ...
                                <CardContent className="flex-1">
                                    <p className="text-sm text-gray-600 line-clamp-3">{assessment.description || "No description provided."}</p>
                                </CardContent>
                                <CardFooter>
                                    <Button
                                        className="w-full"
            ...
            <Dialog open={!!selectedAssessment} onOpenChange={(open) => !open && setSelectedAssessment(null)}>
                <DialogContent className="sm:max-w-[600px]">
                    ...
                    <div className="space-y-4 py-4">
                        <div className="space-y-2">
                            ...
                            <Textarea
                                ...
                                rows={6}
                                disabled={getSubmission(selectedAssessment?.id!)?.status === 'graded'}
                            />
                        </div>
                        <div className="space-y-2">
                        ...
                        {getSubmission(selectedAssessment?.id!)?.status === 'graded' && (
                            <div className="bg-green-50 p-4 rounded-md">
                                ...
                                <p className="text-sm text-green-700">{getSubmission(selectedAssessment?.id!)?.feedback || "No feedback provided."}</p>
                            </div>
                        )}
                    </div>
                    ...
                    <DialogFooter>
                        ...
                        {getSubmission(selectedAssessment?.id!)?.status !== 'graded' && (
                            <Button onClick={handleSubmit} disabled={isSubmitting} className="bg-[#3e6253] hover:bg-[#2c463b]">
