        ...
        <Popover>
            <PopoverTrigger asChild>
                <Button
                    variant={"outline"}
                    className={cn(
                        "h-14 w-full justify-start text-left font-bold px-6 rounded-2xl bg-slate-50 border-2 border-slate-100 hover:border-violet-500/30 hover:bg-white transition-all group",
                        !date && "text-slate-400",
                        className
                    )}
                >
                    <CalendarIcon className="mr-3 h-5 w-5 text-slate-400 group-hover:text-violet-500 transition-colors" />
                ...
                <div className="flex items-center gap-2 p-3 bg-slate-50 border-b border-slate-100">
                    ...
                    >
                        ...
                        <SelectContent className="rounded-lg border-slate-200 shadow-lg max-h-[200px]">
                            {years.map((year) => (
                                <SelectItem key={year} value={year.toString()} className="font-medium text-sm hover:bg-violet-50 hover:text-violet-700 focus:bg-violet-50 focus:text-violet-700 cursor-pointer">
                                    {year}
                                </SelectItem>
                            ))}
                        </SelectContent>
                    ...
                    >
                        <SelectTrigger className="h-9 w-[110px] bg-white border-slate-200 rounded-lg font-semibold text-sm focus:ring-0 focus:border-violet-400">
                            <SelectValue />
                        </SelectTrigger>
                        <SelectContent className="rounded-lg border-slate-200 shadow-lg">
                            {months.map((month) => (
                                <SelectItem key={month} value={month} className="font-medium text-sm hover:bg-violet-50 hover:text-violet-700 focus:bg-violet-50 focus:text-violet-700 cursor-pointer">
                                    {month}
                                </SelectItem>
                            ))}
                        </SelectContent>
