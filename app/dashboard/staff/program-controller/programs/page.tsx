...
const getDepartmentName = (program: Program): string => {
    if (typeof program.departmentId === "object" && program.departmentId?.name)
        return program.departmentId.name;
    return "N/A";
};

const getDepartmentId = (program: Program): string => {
    if (typeof program.departmentId === "string") return program.departmentId;
    if (typeof program.departmentId === "object" && program.departmentId?.id)
        return program.departmentId.id;
    return "";
};

export default function ProgramControllerProgramsPage() {
    ...
    const handleFormSubmit = async (data: Record<string, string>) => {
        ...
        try {
            ...
            if (!duration || duration < 1 || duration > 10) {
                toast.error("Duration must be between 1 and 10 years");
                setIsSubmitting(false);
                return;
            }

            const totalCredits = Number(data.totalCredits);
            if (!totalCredits || totalCredits < 10) {
                toast.error("Total credits must be at least 10");
                setIsSubmitting(false);
                return;
            }
