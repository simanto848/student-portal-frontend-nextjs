...
export default function PrerequisiteDetailsPage() {
  ...
  const [prerequisite, setPrerequisite] = useState<CoursePrerequisite | null>(
    null
  );
  ...
  const fetchPrerequisite = async () => {
    ...
    try {
      ...
      toast.error(message);
      router.push("/dashboard/staff/program-controller/prerequisites");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const getCode = (item: unknown): string => {
    if (!item) return "";
  ...
  const getCredit = (item: unknown): string => {
    if (!item) return "N/A";
    if (typeof item === "string") return "N/A";
    if (typeof item === "object" && item !== null && "credit" in item) {
      return String((item as { credit: number }).credit);
    }
    return "N/A";
  };
          ...
          <div className="bg-white rounded-xl shadow-sm border border-[#a3b18a]/20 p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-lg font-medium text-[#344e41]">
                  {getName(prerequisite.courseId)}
                </p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-base font-medium text-[#344e41]">
                    {getCredit(prerequisite.courseId)}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-[#a3b18a]/20 p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-lg font-medium text-[#344e41]">
                  {getName(prerequisite.prerequisiteId)}
                </p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-base font-medium text-[#344e41]">
                    {getCode(prerequisite.prerequisiteId)}
                  </p>
                </div>
                <div>
                  <label className="text-sm font-medium text-[#344e41]/70">
                  ...
                  <p className="text-base font-medium text-[#344e41]">
                    {getCredit(prerequisite.prerequisiteId)}
                  </p>
                </div>
              </div>
            </div>
