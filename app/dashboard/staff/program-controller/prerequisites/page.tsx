...
export default function CoursePrerequisiteManagementPage() {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      const message =
        error instanceof AcademicApiError ? error.message : "Failed to delete prerequisite";
      toast.error(message);
    } finally {
      setIsDeleting(false);
      setSelectedPrerequisite(null);
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <PageHeader
          ...
          subtitle="Manage course dependencies for your department"
        ...
        {isLoading ? (
          ...
          <DataTable
            data={prerequisites.map((p) => ({
              ...p,
              courseName: (p.courseId as { name?: string })?.name || "N/A",
              prerequisiteName:
                (p.prerequisiteId as { name?: string })?.name || "N/A",
            }))}
            columns={columns}
            ...
            onView={(item) =>
              router.push(
                `/dashboard/staff/program-controller/prerequisites/${item.id}`
              )
            }
            onEdit={handleEdit}
