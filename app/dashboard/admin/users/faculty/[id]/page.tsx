    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          ...
          />
          <Button variant="outline" size="sm" onClick={handleDeleteTeacher} disabled={isDeleting} className="ml-auto border-red-500 text-red-600">
        ...
        <div className="grid gap-6 md:grid-cols-3">
                  ...
                  <div className="flex items-center justify-between pb-3 border-b border-[#a3b18a]/30">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-[#344e41]/60">Extended personal details</p>
                      </div>
                    </div>
                        ...
                        <div className="space-y-2">
                          {profile.addresses.map((addr, idx) => (
                            <div key={idx} className="p-3 rounded-lg bg-white/70 border border-[#a3b18a]/30">
                              <p className="text-sm font-medium text-[#344e41]">{[addr.street, addr.city, addr.state].filter(Boolean).join(', ') || '(No street)'}</p>
                              <p className="text-xs text-[#344e41]/70">{[addr.country, addr.zipCode].filter(Boolean).join(' - ') || '(No country)'} {addr.isPrimary && <span className="ml-2 inline-block px-2 py-0.5 text-[10px] rounded bg-[#588157] text-white">PRIMARY</span>}</p>
                            </div>
                          ))}
                        </div>
                      </div>
              ...
              {!profile && (
                <div className="bg-[#dad7cd]/60 rounded-lg p-5 border border-[#a3b18a]/20">
                  <div className="flex items-start gap-3">
                    <div className="p-2 rounded-lg bg-[#344e41]/10"><UserIcon className="h-5 w-5 text-[#344e41]/60" /></div>
                    <div className="flex-1">
                      <p className="text-sm font-medium text-[#344e41] mb-1">No Profile Information</p>
                      <p className="text-xs text-[#344e41]/60 mb-3">This teacher doesn't have an extended profile yet.</p>
                      <Button variant="outline" size="sm" className="border-[#588157] text-[#588157] hover:bg-[#588157] hover:text-white" onClick={() => router.push(`/dashboard/admin/users/faculty/${teacher.id}/edit`)}>
          ...
          <Card className="border-[#a3b18a]/30">
            <CardContent className="p-6 space-y-4">
              <p className="text-xs font-semibold uppercase tracking-wide text-[#344e41]/70">Registered IP Addresses</p>
              <div className="flex gap-2">
                <Input value={ipInput} onChange={(e) => setIpInput(e.target.value)} placeholder="Add IP" className="bg-white border-[#a3b18a]/60 text-[#344e41]" />
                <Button onClick={addIp} disabled={isIpUpdating} className="bg-[#588157] text-white">Add</Button>
              </div>
...
function InfoRow({ icon: Icon, label, value }: { icon: any; label: string; value: string }) {
  return (
    <div className="space-y-1">
      <p className="text-xs uppercase tracking-wide text-[#344e41]/50">{label}</p>
...
function ProfileField({ label, value, highlighted, icon: Icon, mono }: { label: string; value: string; highlighted?: boolean; icon?: any; mono?: boolean }) {
  return (
    <div className={`text-sm ${highlighted ? 'font-semibold text-[#344e41]' : 'text-[#344e41]/80'} ${mono ? 'font-mono' : ''} flex items-center gap-2`}>
