...
import { academicService, CoursePrerequisite, Course, AcademicApiError } from "@/services/academic.service";
...
export default function CoursePrerequisiteManagementPage() {
    ...
    const [selectedPrerequisite, setSelectedPrerequisite] = useState<CoursePrerequisite | null>(null);
    ...
    const handleFormSubmit = async (data: Record<string, string>) => {
        ...
        try {
            if (!data.courseId || !data.prerequisiteId) {
                toast.error("Course and Prerequisite Course are required");
                setIsSubmitting(false);
                return;
            }

            if (data.courseId === data.prerequisiteId) {
                toast.error("Course cannot be its own prerequisite");
                setIsSubmitting(false);
                return;
            }
            ...
            if (selectedPrerequisite) {
                await academicService.updatePrerequisite(selectedPrerequisite.id, submitData);
        ...
        <DashboardLayout>
            <div className="space-y-6">
                ...
                {isLoading ? (
                    ...
                    <DataTable
                        data={prerequisites.map(p => ({
                            ...p,
                            courseName: (p.courseId as any)?.name || "N/A",
                            prerequisiteName: (p.prerequisiteId as any)?.name || "N/A"
                        }))}
                        columns={columns}
                ...
                <GenericFormModal
                    ...
                    description={selectedPrerequisite ? "Update prerequisite information" : "Add a new course prerequisite"}
                    fields={formFields}
                    initialData={selectedPrerequisite ? {
