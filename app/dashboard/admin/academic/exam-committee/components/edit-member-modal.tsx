...
export function EditMemberModal({ member, open, onClose, onSuccess }: EditMemberModalProps) {
    ...
    const fetchBatches = async () => {
        try {
            ...
            const deptId = typeof member?.departmentId === 'object' ? member.departmentId.id : member?.departmentId;
            const filtered = deptId
                ? response.filter(b => (typeof b.departmentId === 'object' ? b.departmentId.id : b.departmentId) === deptId)
                : response;
            setBatches(filtered);
        } catch (error) {
    ...
    const onSubmit = async (values: z.infer<typeof formSchema>) => {
        ...
        try {
            const payload = {
                batchId: values.batchId === 'null' || !values.batchId ? 'null' : values.batchId,
                ...
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                        <div className="text-sm text-muted-foreground mb-4">
                            Editing member: <span className="font-semibold">{member.teacher?.fullName}</span>
                        </div>
