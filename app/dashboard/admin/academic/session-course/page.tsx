...
export default function SessionCourseManagementPage() {
  ...
  const handleFormSubmit = async (data: Record<string, unknown>) => {
    try {
      ...
      const courseIds = Array.isArray(data.courseId)
        ? data.courseId
        : [data.courseId];
      ...
      notifySuccess("Session courses saved successfully");
    ...
    return sessionCourses.filter((item) => {
      const sessionMatch =
        ...
        (typeof item.sessionId === "object"
          ? item.sessionId.id === selectedSession
          : item.sessionId === selectedSession);
      const deptMatch =
        selectedDepartment === "all" ||
        (typeof item.departmentId === "object"
          ? item.departmentId.id === selectedDepartment
