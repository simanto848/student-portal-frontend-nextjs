...
import { Loader2, Plus, Search, GraduationCap, BookOpen, Users, ChevronDown, ChevronUp, University, Calendar } from "lucide-react";
...
export default function EnrollmentsPage() {
    ...
    const groupedEnrollments: StudentEnrollmentGroup[] = enrollments.reduce((acc, enrollment) => {
        ...
        if (existing) {
            ...
            if ((enrollment.status === 'active' || enrollment.status === 'enrolled') && enrollment.semester === existing.semester) {
            ...
            acc.push({
                ...
                enrollments: [enrollment],
                totalCourses: 1,
                activeCount: ((enrollment.status === 'active' || enrollment.status === 'enrolled') && enrollment.semester === enrollment.semester) ? 1 : 0,
    ...
    const filteredGroups = groupedEnrollments.filter(group =>
        group.student?.fullName?.toLowerCase().includes(search.toLowerCase()) ||
        group.student?.registrationNumber?.toLowerCase().includes(search.toLowerCase()) ||
        group.batch?.name?.toLowerCase().includes(search.toLowerCase())
    );
        ...
        <DashboardLayout>
            <div className="space-y-6">
                ...
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-3xl font-bold tracking-tight text-[#1a3d32]">Student Enrollments</h1>
                        <p className="text-muted-foreground">Comprehensive view of student course enrollments</p>
                    </div>
                ...
                <div className="grid gap-4 md:grid-cols-3">
                    <Card key="total-students">
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold text-[#1a3d32]">{totalStudents}</div>
                        </CardContent>
                    </Card>
                    <Card key="total-enrollments">
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                            <CardTitle className="text-sm font-medium">Total Course Enrollments</CardTitle>
                            <BookOpen className="h-4 w-4 text-[#344e41]" />
                        </CardHeader>
                        <CardContent>
                            <div className="text-2xl font-bold text-[#1a3d32]">{totalEnrollments}</div>
                        </CardContent>
                    </Card>
                    <Card key="unique-courses">
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                            <CardTitle className="text-sm font-medium">Unique Courses</CardTitle>
                            <GraduationCap className="h-4 w-4 text-[#344e41]" />
                        </CardHeader>
                        <CardContent>
                            <div className="text-2xl font-bold text-[#1a3d32]">{uniqueCourses}</div>
                        </CardContent>
                    </Card>
                </div>
                                    ...
                                    <div className="flex items-start justify-between">
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-sm text-muted-foreground">
                                                        {group.student?.registrationNumber || 'No registration number'}
                                                    </p>
                                                </div>
                                            </div>
                                            ...
                                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
                                                <div key="department" className="flex items-center gap-2">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-sm font-medium">
                                                            {group.batch?.departmentId?.shortName || 'N/A'}
                                                        </p>
                                                    </div>
                                                </div>
                                                <div key="batch" className="flex items-center gap-2">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-sm font-medium">{group.batch?.name || 'N/A'}</p>
                                                    </div>
                                                </div>
                                                <div key="session" className="flex items-center gap-2">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-sm font-medium">
                                                            {typeof group.session === 'object' ? group.session?.name : group.session || 'N/A'}
                                                        </p>
                                                    </div>
                                                </div>
                                                <div key="semester" className="flex items-center gap-2">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-sm font-medium">Semester {group.semester}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex flex-col items-end gap-2">
                                            <div className="text-right">
                                                <p className="text-2xl font-bold text-[#1a3d32]">{group.totalCourses}</p>
                                                <p className="text-xs text-muted-foreground">Total Courses</p>
                                            </div>
                                            <div className="flex gap-2">
                                                <Badge key="active" className="bg-green-100 text-green-800 hover:bg-green-100">
                                                    {group.activeCount} Active
                                                </Badge>
                                                {group.completedCount > 0 && (
                                                    <Badge key="completed" className="bg-blue-100 text-blue-800 hover:bg-blue-100">
                                            ...
                                            >
                                                ...
                                                {expandedStudent === group.studentId ? 'Hide Courses' : 'View Courses'}
                                            </Button>
                                        </div>
                                    </div>
                                </CardHeader>
                                            ...
                                            {group.enrollments.map((enrollment, idx) => (
                                                ...
                                                >
                                                        ...
                                                        <div>
                                                            ...
                                                            <p className="text-sm text-muted-foreground">
                                                                ...
                                                                {enrollment.instructor && (
                                                                    <span className="ml-2">
                                                                        • Instructor: {enrollment.instructor?.fullName}
                                                                    </span>
                                                                )}
                                                            </p>
                                                        </div>
                                                    ...
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        >
                                                            {enrollment.status === 'active' ? 'Active' : enrollment.status.charAt(0).toUpperCase() + enrollment.status.slice(1)}
                                                        </Badge>
                    ...
                    <Card>
                        <CardContent className="flex flex-col items-center justify-center py-12">
                            ...
                            <p className="text-sm text-muted-foreground mt-1">
                                {search ? "Try adjusting your search" : "Start by enrolling students"}
                            </p>
                        </CardContent>
                    </Card>
                )}
