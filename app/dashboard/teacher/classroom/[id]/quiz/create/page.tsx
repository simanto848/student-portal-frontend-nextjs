...
const QUESTION_TYPES = [
    ...
    { value: "mcq_multiple", label: "Multiple Choice (Multiple Answers)", icon: "☑️" },
        ...
        return (
            <Card key={question.tempId} className="overflow-hidden">
                <div
                    ...
                    onClick={() => setExpandedQuestion(isExpanded ? null : question.tempId)}
                >
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">{typeInfo?.label} • {question.points} pts</p>
                    </div>
                    <Button
                        variant="ghost"
                        size="icon"
                        className="text-red-500 hover:text-red-600 hover:bg-red-50"
                        onClick={(e) => { e.stopPropagation(); removeQuestion(question.tempId); }}
                    >
                            ...
                            {(question.type?.startsWith("mcq") || question.type === "true_false") && (
                                <div className="space-y-3">
                                    ...
                                    <p className="text-xs text-muted-foreground">
                                        {isMultipleChoice ? "Select all correct answers" : "Select the correct answer"}
                                    </p>
                                    {question.options?.map((option, idx) => (
                                        <div key={option.id} className="flex items-center gap-2">
                                            <button
                                                type="button"
                                                onClick={() => setCorrectOption(question.tempId, option.id, isMultipleChoice)}
                                                className={cn(
                                            ...
                                            <Input
                                                ...
                                                value={option.text}
                                                onChange={(e) => updateOption(question.tempId, option.id, { text: e.target.value })}
                                                className="flex-1"
            ...
            <div className="max-w-4xl mx-auto space-y-6 pb-8">
                ...
                <div className="flex items-center gap-4">
                    <Button
                        variant="outline"
                        size="icon"
                        onClick={() => router.push(`/dashboard/teacher/classroom/${workspaceId}/quiz`)}
                ...
                <div className="bg-white rounded-xl p-4 shadow-sm border">
                    ...
                    <div className="text-center text-sm text-muted-foreground">
                        Step {step} of 4: <span className="font-medium text-[#344e41]">{steps[step - 1].title}</span>
                    </div>
                </div>
                ...
                <Card>
                    <CardContent className="pt-6">
                        {step === 1 && (
                            <div className="space-y-6">
                                ...
                                <div className="border-t pt-4 mt-4">
                                    <h3 className="text-sm font-semibold text-[#344e41] mb-3">Quiz Scheduling (Optional)</h3>
                                    <p className="text-xs text-muted-foreground mb-4">
                                        Set when this quiz becomes available to students. Leave empty for no time restrictions.
                                    </p>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <Input
                                                ...
                                                className="mt-1.5"
                                            />
                                            <p className="text-xs text-muted-foreground mt-1">Quiz opens at this time</p>
                                        </div>
                                        <div>
                                            ...
                                            <Input
                                                ...
                                                className="mt-1.5"
                                            />
                                            <p className="text-xs text-muted-foreground mt-1">Quiz closes at this time</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {QUESTION_TYPES.map((type) => (
                                        ...
                                        >
                                            ...
                                            <span className="sm:hidden">{type.value.split("_")[0].toUpperCase()}</span>
                                        </Button>
                                    ))}
                                </div>
                                ...
                                {questions.length === 0 ? (
                                    <div className="text-center py-12 border-2 border-dashed rounded-lg">
                                        ...
                                        <h3 className="text-lg font-medium text-[#344e41] mb-2">No questions yet</h3>
                                ...
                                {questions.length > 0 && (
                                    <div className="bg-muted/30 rounded-lg p-4">
                                        <p className="text-sm text-muted-foreground">
                                            <strong className="text-[#344e41]">{questions.length}</strong> questions •
                                            <strong className="text-[#344e41] ml-1">{questions.reduce((sum, q) => sum + (q.points || 0), 0)}</strong> total points
                                        </p>
                                    </div>
                                )}
                            </div>
                        ...
                        {step === 3 && (
                            <div className="space-y-6">
                                <div className="grid gap-4">
                                    ...
                                    <div className="flex items-center justify-between p-4 border rounded-lg">
                                        <div>
                                            <Label className="font-medium">Show Results After Submit</Label>
                                            <p className="text-sm text-muted-foreground">
                        ...
                        {step === 4 && (
                            <div className="space-y-6">
                                <h3 className="text-lg font-semibold text-[#344e41]">Review Your Quiz</h3>
                                ...
                                <div className="grid gap-4">
                                    <Card className="bg-muted/30">
                                        ...
                                        <CardContent className="text-sm space-y-1">
                                            ...
                                            {startAt && <p><strong>Starts:</strong> {new Date(startAt).toLocaleString()}</p>}
                                            {endAt && <p><strong>Ends:</strong> {new Date(endAt).toLocaleString()}</p>}
                                            {!startAt && !endAt && <p className="text-muted-foreground italic">No time restrictions set</p>}
                                        </CardContent>
                                    </Card>

                                    <Card className="bg-muted/30">
                                        <CardHeader className="pb-2">
                                            <CardTitle className="text-base">Questions Summary</CardTitle>
                                        </CardHeader>
                                        <CardContent className="text-sm space-y-1">
                                            ...
                                            <div className="flex flex-wrap gap-2 mt-2">
                                                {QUESTION_TYPES.map(type => {
                                                    ...
                                                    return (
                                                        <span key={type.value} className="bg-white px-2 py-1 rounded text-xs">
                                    ...
                                    <Card className="bg-muted/30">
                                        ...
                                        <CardContent className="text-sm space-y-1">
                                            <p><strong>Shuffle Questions:</strong> {shuffleQuestions ? "Yes" : "No"}</p>
                                            <p><strong>Shuffle Options:</strong> {shuffleOptions ? "Yes" : "No"}</p>
