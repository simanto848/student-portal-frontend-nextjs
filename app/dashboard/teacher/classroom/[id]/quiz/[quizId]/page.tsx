        ...
        <DashboardLayout>
            <div className="max-w-6xl mx-auto space-y-6 pb-8">
                ...
                <nav className="flex items-center gap-2 text-sm text-muted-foreground">
                    ...
                    <ChevronRight className="h-4 w-4" />
                    <span className="text-[#344e41] font-medium truncate max-w-[200px]">{quiz.title}</span>
                </nav>
                    ...
                    <div className={cn("bg-gradient-to-r p-6 text-white", status.gradient)}>
                        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                                {quiz.status === "draft" && (
                                    <>
                                        <Link href={`/dashboard/teacher/classroom/${workspaceId}/quiz/${quizId}/edit`}>
                                ...
                                <CardContent className="pt-6">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-2xl font-bold text-[#344e41]">{submissions.length}</p>
                                        </div>
                                    </div>
                                </CardContent>
                            </Card>
                                ...
                                <CardContent className="pt-6">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-2xl font-bold text-[#344e41]">{avgScore}%</p>
                                        </div>
                                    </div>
                                </CardContent>
                            </Card>
                                ...
                                <CardContent className="pt-6">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-2xl font-bold text-[#344e41]">{passedCount}</p>
                                        </div>
                                    </div>
                                </CardContent>
                            </Card>
                                ...
                                <CardContent className="pt-6">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-2xl font-bold text-[#344e41]">{pendingCount}</p>
                                        </div>
                                    </div>
                                </CardContent>
                            </Card>
                                ...
                                <CardContent className="space-y-4">
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="p-3 rounded-lg bg-gray-50">
                                            ...
                                            <p className="text-lg font-semibold text-[#344e41]">{quiz.maxAttempts}</p>
                                        </div>
                                        <div className="p-3 rounded-lg bg-gray-50">
                                            <p className="text-xs text-muted-foreground uppercase tracking-wide">Max Score</p>
                                            <p className="text-lg font-semibold text-[#344e41]">{quiz.maxScore} pts</p>
                                        </div>
                                        <div className="p-3 rounded-lg bg-gray-50">
                                            <p className="text-xs text-muted-foreground uppercase tracking-wide">Passing Score</p>
                                            <p className="text-lg font-semibold text-[#344e41]">
                                                {quiz.passingScore > 0 ? `${quiz.passingScore}%` : "Not set"}
                                            </p>
                                        </div>
                                        <div className="p-3 rounded-lg bg-gray-50">
                                            <p className="text-xs text-muted-foreground uppercase tracking-wide">Time Limit</p>
                                            <p className="text-lg font-semibold text-[#344e41]">{quiz.duration} min</p>
                                        </div>
                                    </div>
                                    <div className="space-y-2 pt-2">
                                        <div className="flex items-center justify-between py-2 border-b">
                                            <span className="text-sm text-muted-foreground">Shuffle Questions</span>
                                            <Badge variant="secondary" className={quiz.shuffleQuestions ? "bg-emerald-100 text-emerald-700" : "bg-gray-100"}>
                                                {quiz.shuffleQuestions ? "Yes" : "No"}
                                            </Badge>
                                        </div>
                                        <div className="flex items-center justify-between py-2 border-b">
                                            <span className="text-sm text-muted-foreground">Shuffle Options</span>
                                            <Badge variant="secondary" className={quiz.shuffleOptions ? "bg-emerald-100 text-emerald-700" : "bg-gray-100"}>
                                                {quiz.shuffleOptions ? "Yes" : "No"}
                                            </Badge>
                                        </div>
                                        <div className="flex items-center justify-between py-2">
                                            <span className="text-sm text-muted-foreground">Show Results</span>
                                            <Badge variant="secondary" className={quiz.showResultsAfterSubmit ? "bg-emerald-100 text-emerald-700" : "bg-gray-100"}>
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="p-3 rounded-lg bg-gray-50">
                                            ...
                                            <p className="text-sm font-semibold text-[#344e41]">
                                                {quiz.startAt ? new Date(quiz.startAt).toLocaleString() : "Not set"}
                                            </p>
                                        </div>
                                        <div className="p-3 rounded-lg bg-gray-50">
                                            <p className="text-xs text-muted-foreground uppercase tracking-wide">End Date</p>
                                            <p className="text-sm font-semibold text-[#344e41]">
                                                {quiz.endAt ? new Date(quiz.endAt).toLocaleString() : "Not set"}
                                            </p>
                                        </div>
                                    </div>
                                    {quiz.instructions ? (
                                        <div className="p-4 rounded-lg bg-blue-50 border border-blue-100">
                                            <p className="text-xs text-blue-600 uppercase tracking-wide font-medium mb-2">Instructions</p>
                                            <p className="text-sm text-blue-800 whitespace-pre-wrap">{quiz.instructions}</p>
                                        </div>
                                    ) : (
                                ...
                                <CardContent>
                                    <div className="space-y-3">
                                        {[
                                            ...
                                            { label: "Below 50%", min: 0, max: 49, color: "bg-rose-500" },
                                        ].map((range) => {
                                            ...
                                            return (
                                                <div key={range.label} className="flex items-center gap-4">
                                                    <span className="text-sm text-muted-foreground w-24">{range.label}</span>
                        ...
                        {questions.length === 0 ? (
                            <Card className="border-dashed border-2 bg-gradient-to-br from-gray-50 to-white">
                                <CardContent className="py-16 text-center">
                                    ...
                                    <h3 className="text-lg font-semibold text-[#344e41] mb-2">No questions yet</h3>
                                    <p className="text-muted-foreground mb-6">Add questions to make this quiz ready for students</p>
                                    {quiz.status === "draft" && (
                                        <Link href={`/dashboard/teacher/classroom/${workspaceId}/quiz/${quizId}/edit`}>
                                                ...
                                                <div className="grid gap-2">
                                                    {question.options.map((option, optIdx) => (
                                                        ...
                                                        >
                                                            ...
                                                            <span className={option.isCorrect ? "font-medium text-emerald-700" : ""}>
                                                                {option.text}
                                                            </span>
                                                            {option.isCorrect && (
