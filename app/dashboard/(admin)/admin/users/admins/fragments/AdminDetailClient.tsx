...
import { AdminProfile } from "@/services/user/adminProfile.service";
import {
    ArrowLeft,
    Shield,
    Mail,
...
import { updateAdminRoleAction, updateAdminIpAction } from "../actions";
...
export function AdminDetailClient({ admin: initialAdmin, profile }: AdminDetailClientProps) {
    ...
    const handleRoleUpdate = async (role: AdminRole) => {
        ...
        try {
            ...
            const result = await updateAdminRoleAction(admin.id, null, formData);
            if (result.success) {
                setAdmin({ ...admin, role });
            ...
            notifyError("A security error occurred");
        } finally {
    ...
    const handleAddIp = async () => {
        ...
        try {
            ...
            const result = await updateAdminIpAction(admin.id, null, formData);
            if (result.success) {
                setAdmin({
                    ...admin,
                    registeredIpAddress: [...(admin.registeredIpAddress || []), newIp.trim()]
                });
                setNewIp("");
            ...
            notifyError("IP registration failure");
        } finally {
    ...
    const handleRemoveIp = async (ip: string) => {
        ...
        try {
            ...
            if (result.success) {
                setAdmin({
                    ...admin,
                    registeredIpAddress: admin.registeredIpAddress?.filter(i => i !== ip) || []
                });
            ...
            notifyError("IP revocation failure");
        } finally {
            setIsIpUpdating(false);
        }
    };
                    ...
                    <div>
                        <div className="flex items-center gap-3 mb-2">
                            <Badge className="bg-amber-100 text-amber-700 border-none px-3 py-1 rounded-full flex items-center gap-2 shadow-sm">
                                ...
                                <span className="text-[10px] font-black uppercase tracking-widest">{roleLabel[admin.role]}</span>
                            </Badge>
                            <span className="text-slate-300 font-black text-xs uppercase tracking-widest">Registration ID: {admin.registrationNumber}</span>
                        </div>
                        <h1 className="text-4xl font-black tracking-tighter text-slate-900 leading-none">{admin.fullName}</h1>
                    </div>
                </div>
                        ...
                        <CardContent className="p-10 pt-16">
                            <div className="flex flex-col sm:flex-row gap-10 items-start">
                                <div className="relative flex-shrink-0 group">
                                    <div className="h-40 w-40 rounded-[2.5rem] overflow-hidden border-4 border-white shadow-2xl relative z-10">
                                ...
                                <div className="flex-1 space-y-8 pt-4">
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                                        <InfoBlock icon={Mail} label="Email Address" value={admin.email} />
                                        <InfoBlock icon={Calendar} label="Joining Date" value={admin.joiningDate ? new Date(admin.joiningDate).toLocaleDateString() : "ETERNAL"} />
                                        <InfoBlock icon={MapPin} label="Registration ID" value={admin.registrationNumber} />
                                        <InfoBlock icon={Smartphone} label="Phone Number" value={profile?.phoneNumber || "N/A"} />
                                    </div>

                                    <div className="pt-8 border-t border-slate-100">
                                        <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-4 px-1">Role Management</p>
                                        <div className="flex flex-col sm:flex-row gap-4">
                                            <Select value={admin.role} onValueChange={(v) => handleRoleUpdate(v as AdminRole)} disabled={isUpdatingRole}>
                                                <SelectTrigger className="h-14 px-6 rounded-2xl bg-slate-50 border-2 border-slate-100 font-bold text-slate-900 w-full sm:w-64 focus:ring-amber-500/20">
                                                    <SelectValue placeholder="Select Role" />
                                                </SelectTrigger>
                                                <SelectContent className="rounded-2xl border-2 border-slate-100 shadow-2xl">
                                                    {Object.entries(roleLabel).map(([key, label]) => (
                                                        <SelectItem key={key} value={key} className="py-3 font-bold text-slate-700 focus:bg-amber-50 focus:text-amber-700 rounded-xl m-1">
                                                            {label}
                                                        </SelectItem>
                                                    ))}
                                                </SelectContent>
                                            </Select>
                                            <div className="flex-1 flex items-center gap-3 px-4 py-3 rounded-2xl bg-amber-50/50 border border-amber-100 text-[10px] font-bold text-amber-700 italic">
                                                <Clock className="w-3 h-3" />
                                                Higher roles grant more administrative permissions.
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <Card className="bg-white border-2 border-slate-100 rounded-[3rem] shadow-2xl shadow-slate-200/40 relative group overflow-hidden">
                                ...
                                <CardContent className="p-10">
                                    <div className="flex items-center gap-4 mb-10">
                                        ...
                                        <div>
                                            <h2 className="text-2xl font-black text-slate-900">Personal Info</h2>
                                            <p className="text-slate-500 font-bold text-sm italic">Additional personal details and identification info.</p>
                                        </div>
                                    </div>
                                    ...
                                    {profile.addresses && profile.addresses.length > 0 && (
                                        <div className="mt-12 pt-10 border-t border-slate-100">
                                            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-6 px-1">Registered Addresses</p>
                                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                                {profile.addresses.map((addr, idx) => (
                                                    <div key={idx} className="p-6 rounded-3xl bg-slate-50/50 border-2 border-slate-100 hover:border-amber-500/20 transition-colors group">
                                                        <div className="flex items-start justify-between mb-3">
                                                            <div className="h-8 w-8 rounded-xl bg-white flex items-center justify-center text-slate-400 group-hover:text-amber-600 transition-colors">
                    ...
                    <Card className="bg-slate-900 text-white border-none rounded-[3rem] shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5)] relative overflow-hidden group">
                        ...
                        <CardContent className="p-10 relative z-10">
                            <div className="flex items-center justify-between mb-8">
                                <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-500">Account Status</p>
                            ...
                            <p className="text-emerald-400 font-bold text-xs uppercase tracking-widest flex items-center gap-2">
                                ...
                                Last Login IP: {admin.lastLoginIp || "UNKNOWN"}
                            </p>

                            <div className="grid grid-cols-2 gap-4 mt-12 bg-white/5 p-6 rounded-[2rem] backdrop-blur-md">
                                <div>
                                    <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1">Created Date</p>
                                    <p className="text-[10px] font-black">{new Date(admin.createdAt || "").toLocaleDateString()}</p>
                                </div>
                                <div>
                                    <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1">State Version</p>
                                    <p className="text-[10px] font-black">v1.0.RC94</p>
                                </div>
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <Card className="bg-white border-2 border-slate-100 rounded-[3rem] shadow-2xl shadow-slate-200/40 overflow-hidden group">
                        <CardContent className="p-10">
                            <div className="flex items-center gap-4 mb-8">
                                ...
                                <div>
                                    <h3 className="text-xl font-black text-slate-900">Access Rules</h3>
                                    <p className="text-slate-400 font-bold text-[10px] uppercase tracking-widest">Restricted IP Addresses</p>
                                </div>
                            </div>
                            ...
                            <div className="space-y-6">
                                <div className="flex gap-2 relative">
                                    <Input
                                        placeholder="Add allowed IP address..."
                                        value={newIp}
                                ...
                                <div className="flex flex-wrap gap-2 min-h-[50px]">
                                    {admin.registeredIpAddress && admin.registeredIpAddress.length > 0 ? (
                                        ...
                                        <div className="w-full py-8 text-center bg-slate-50 rounded-[2rem] border-2 border-dashed border-slate-100 flex flex-col items-center gap-2">
                                            ...
                                            <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest leading-none">Access from any IP permitted</p>
                                        </div>
                                    )}
                                </div>
                            </div>
