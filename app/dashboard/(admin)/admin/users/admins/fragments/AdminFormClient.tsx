...
import { Admin, AdminRole, adminService } from "@/services/user/admin.service";
...
export function AdminFormClient({ admin, profile }: AdminFormClientProps) {
    ...
    const [basic, setBasic] = useState({
        ...
        email: admin?.email || "",
        role: admin?.role || "moderator" as AdminRole,
        registrationNumber: admin?.registrationNumber || "",
    });

    const [advanced, setAdvanced] = useState({
        joiningDate: admin?.joiningDate ? admin.joiningDate.split("T")[0] : "",
        registeredIps: admin?.registeredIpAddress || [],
        ipInput: "",
    });
    ...
    const [previewUrl, setPreviewUrl] = useState<string>(admin?.profile?.profilePicture ? (getImageUrl(admin.profile.profilePicture) ?? "") : "");

    useEffect(() => {
        if (!profilePicture) {
            if (isEdit && admin?.profile?.profilePicture) {
                setPreviewUrl(getImageUrl(admin.profile.profilePicture) ?? "");
            } else {
                setPreviewUrl("");
            }
            return;
    ...
    }, [profilePicture, isEdit, admin, getImageUrl]);
    ...
    const handleAddIp = () => {
        ...
        if (advanced.registeredIps.includes(ip)) {
            toast.error("IP already registered");
            return;
        }
    ...
    const addAddress = () => {
        if (!addressDraft.street && !addressDraft.city && !addressDraft.country) {
            toast.error("Please provide street, city, and country");
            return;
        }
        setAddresses(prev => {
            let next = [...prev];
    ...
    const steps = [
        ...
        { id: 2, label: "Access Rules", icon: Lock },
                                    ...
                                    <FormGroup label="Admin Role" icon={Shield}>
                                        <Select value={basic.role} onValueChange={(v) => setBasic({ ...basic, role: v as AdminRole })}>
                                            ...
                                            <SelectContent className="rounded-2xl border-2 border-slate-100 shadow-2xl">
                                                <SelectItem value="super_admin" className="py-3 font-bold text-slate-700 m-1 rounded-xl">Super Admin</SelectItem>
                                                <SelectItem value="admin" className="py-3 font-bold text-slate-700 m-1 rounded-xl">Admin</SelectItem>
                                                <SelectItem value="moderator" className="py-3 font-bold text-slate-700 m-1 rounded-xl">Moderator</SelectItem>
                                            </SelectContent>
                                        </Select>
                                    </FormGroup>
                        ...
                        {step === 2 && (
                            <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 lg:gap-8">
                                    ...
                                    <FormGroup label="Access Rules (IP)" icon={Lock}>
                                        <div className="relative">
                                            <Input
                            ...
                            <motion.div key="step4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <div className="space-y-8">
                                        <FormGroup label="Street" icon={Globe}>
                                ...
                                <div className="bg-amber-50 p-8 rounded-[2.5rem] border-2 border-amber-100 space-y-8">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-amber-700 font-bold text-xs italic">Please review the information below before saving the admin profile.</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 lg:gap-8 pt-4">
                                        ...
                                        <SummaryItem label="Registration ID" value={basic.registrationNumber} />
                                        <SummaryItem label="Joining Date" value={advanced.joiningDate || "NOT SET"} />
                                    ...
                                    {!isEdit && (
                                        <div className="p-6 rounded-3xl bg-slate-900 text-white flex items-start gap-4">
                                            ...
                                            <p className="text-[11px] font-bold leading-relaxed text-slate-300">
                                                A temporary password will be sent to the admin's email address upon creation.
