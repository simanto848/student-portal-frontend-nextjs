...
export function StaffFormClient({ staff, profile, departments }: StaffFormClientProps) {
    ...
    const handleSubmit = async () => {
        if (!basic.fullName || !basic.departmentId || (!isEdit && !basic.email)) {
            toast.error("Required fields missing in Identity phase");
            setStep(1);
            return;
        }
                            ...
                            <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    ...
                                    <FormGroup label="Department" icon={Building2}>
                                        <Select value={basic.departmentId} onValueChange={(v) => setBasic({ ...basic, departmentId: v })}>
                                        ...
                                        <Select value={basic.role} onValueChange={(v) => setBasic({ ...basic, role: v as StaffRole })}>
                                            ...
                                            <SelectContent className="rounded-2xl border-2 border-slate-100 shadow-2xl">
                                                {Object.entries(roleLabel).map(([key, label]) => (
                                                    <SelectItem key={key} value={key} className="py-3 font-bold text-slate-700 m-1 rounded-xl">{label}</SelectItem>
                                                ))}
                                            </SelectContent>
                                        </Select>
                                    </FormGroup>
                            ...
                            <motion.div key="step3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                                    ...
                                    <FormGroup label="Phone Number" icon={Network}>
                                        <Input value={profileForm.phoneNumber} onChange={e => setProfileForm({ ...profileForm, phoneNumber: e.target.value })} placeholder="+1 (XXX) XXX-XXXX" className="h-14 px-6 rounded-2xl bg-slate-50 border-2 border-slate-100 font-bold text-slate-900" />
                                    </FormGroup>
                                    <FormGroup label="Gender" icon={Briefcase}>
