...
export default function PasswordChangePage() {
    const [pwd, setPwd] = useState("Str0ngP@ss!");
    const [confirm, setConfirm] = useState("Str0ngP@ss!");
            ...
            <div className="flex h-full grow flex-col">
                <header className="flex items-center justify-between whitespace-nowrap border-b border-solid border-border-alt-light dark:border-border-alt-dark/50 px-6 sm:px-10 md:px-20 py-4 bg-white dark:bg-background-dark">
                    <div className="flex items-center gap-4 text-text-light dark:text-text-dark">
                        ...
                        <h2 className="text-lg font-bold leading-tight tracking-[-0.015em]">Student Portal</h2>
                    </div>
                    <div className="flex flex-1 justify-end gap-8">
                        <div className="bg-center bg-no-repeat aspect-square bg-cover rounded-full size-10" style={{ backgroundImage: 'url("https://lh3.googleusercontent.com/aida-public/AB6AXuBy4g3oD8EvZCBbe6sujb1QHRUsQPhECLD44lginBEl3pt0YBi9nlePxkk_k2YNVMiQ3LzET1uMkYqpcNAhTpdTxz1woljZnIZehMNn1hHxSD9IuNEMAyjXt8hkdVBrHFNc9lDxk-L7RUpmlyjK3moaGj6NhtT_rP76_iRiUiSQNHdEeizEcIkWxRfabXhKh9kX83j9kFkaHRmKAHI_cCIXVNFhrNeSwNaxCgOUaclWpdBzUF6ux9qsvqUtXfUu4ZVJilpMWFtrUtw")' }} />
                    </div>
                </header>
                    ...
                    <div className="w-full max-w-2xl">
                        <Card className="bg-white dark:bg-background-dark/70 rounded-xl border-border-alt-light dark:border-border-alt-dark p-6 sm:p-10">
                            <div className="flex flex-col gap-2 mb-8">
                                <h1 className="text-text-light dark:text-text-dark text-4xl font-black leading-tight tracking-[-0.033em]">Change Your Password</h1>
                                <p className="text-subtext-light dark:text-subtext-dark text-base">Create a new, secure password for your account.</p>
                            </div>
                            <form className="flex flex-col gap-6" onSubmit={(e) => { e.preventDefault(); if (canSubmit) { /* submit */ } }}>
                                <PasswordField label="New Password" value={pwd} onChange={setPwd} placeholder="Enter your new password" autoComplete="new-password" />
                                <PasswordStrengthIndicator password={pwd} />
                                <PasswordField label="Confirm New Password" value={confirm} onChange={setConfirm} placeholder="Confirm your new password" autoComplete="new-password" />
